    ...
    <div class="menuList">
      ...
      <ul class="list" @mouseenter="changeChildList(1)" @mouseleave="changeChildList(2)">
        <li v-for="(item, index) in menue" :key="index" @mouseenter="addChildList(item.children, index, item.id)"
          ...
          <img class="iconActivity" v-if="item.id == 0" :src="require('@/assets/image/icon_fire.gif')">
          <span>{{ item.name }}</span>
          <!-- <span :class="[item.id == curIndex ? 'showUnder' : '']" class="underline"></span> -->
        </li>
        <li style="position: relative; z-index: 9; lfet: 0; top: 0px" v-show="$common.getUser()">
          <search-item></search-item>
        </li>
      </ul>
      <div class="wrapper" v-show="childSwitch && childList.length > 0" @mouseenter="changeChildList(1)" @mouseleave="changeChildList(2)">
        <div class="childList">
          <ul class="right">
            <li v-for="(item, index) in childList" :class="getGameClas(childList)" :key="index" @click="jump(item)">
              <img :src="$config.imgHost + item.imgUrl" alt :onerror="noData" class="img_icon"/>
              <div class="listLogo">
                <img class="img_logo" :src="$config.imgHost + item.imgUrlOne" alt="">
...
export default {
  ...
  methods: {
    ...
    gameMenuSort() {
      ...
      // this.menue.unshift({ name: this.$t('首页'),id: 0,})
    ...
    .childList {
      ...
      .right {
        ...
        li {
          ...
          overflow: hidden;
          &:hover {
            background: -moz-linear-gradient(top, rgba(219,232,251,1) 0%, rgba(219,232,251,0.2) 100%);
            background: -webkit-linear-gradient(top, rgba(219,232,251,1) 0%,rgba(219,232,251,0.2) 100%);
            background: linear-gradient(to bottom, rgba(219,232,251,1) 0%,rgba(219,232,251,0.2) 100%);
