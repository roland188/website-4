<template>
  <view class="botion_component"> </view>
</template>
<script>
export default {
  data() {
  ...
  methods: {
    showCaptcha() {
      ...
      this.wv = plus.webview.create(
        `hybrid/html/botion/boc-index.html?data=${encodeURIComponent(
      ...
      // 获取webview
      var currentWebview = that.$root.$scope.$getAppWebview(); //此对象相当于html5plus里的plus.webview.currentWebview()。在uni-app里vue页面直接使用plus.webview.currentWebview()无效
