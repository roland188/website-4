    ...
    <div class="securityCenter personal">
      <div>
          ...
          <div class="perRow">
              ...
              :placeholder="$t('请输入新密码')"
              @blur="confirmNewPwdBlur"
              />
          </div>
          <!-- 提交 -->
          <div class="perSubmit">
              <div
              class="u-flex-all themeColorkBgc"
              @click="updatePassword"
              ><el-button type="primary" round style="width: 100%;" :disabled='upPwdDisabled ? false :true'>{{$t('提交')}}</el-button></div>
          </div>
          </div>
      </div>
    </div>
...
export default {
  ...
  methods: {
    ...
    pwdIsconfirmPwd() {
      //密码和确认密码是否相同
      return this.newPassword == this.confirmNewPwd;
    },
      //修改登录密码
    async updatePassword() {
      var _this = this;
      if (!this.upPwdDisabled) {
        return;
      }
      //校验
      if (!this.newPasswordIsValid()) {
        this.$message.error(this.$t("密码必须为6-15位字母或数字！"));
        return;
      } else if (!this.pwdIsconfirmPwd()) {
        this.$message.error(this.$t("密码与确认密码不一致！"));
        return;
      }
      //修改登录密码接口
      await _this.updatePasswordInter();
    },
      //修改登录密码接口
    async updatePasswordInter() {
      var _this = this;
      var data = {
        newpassword1: this.newPassword,
        password: this.oldPassword
      };
      var res = await _this.$http.post(_this.$api.updatePassword, data);
      if (res.code == 0) {
        this.$message.success(this.$t("修改登录密码成功"));
        this.oldPassword = ''
        this.newPassword = ''
        this.confirmNewPwd = ''
        this.logout()
    ...
    newPasswordBlur() {
      ...
      //全角转半角
      // this.newPassword = this.toCDB(this.newPassword);
      //空格处理
      // this.newPassword = this.newPassword.replace(/\ /g, "");
    },
    confirmNewPwdBlur() {
      //大写转小写
      // this.confirmNewPwd = this.confirmNewPwd.toLowerCase();
      //全角转半角
      // this.confirmNewPwd = this.toCDB(this.confirmNewPwd);
      //空格处理
      // this.confirmNewPwd = this.confirmNewPwd.replace(/\ /g, "");
    }
  },
  computed: {
    upPwdDisabled() {
      return this.oldPassword && this.newPassword && this.confirmNewPwd;
    }
  },
};
</script>
