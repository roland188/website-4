<template>
  <view class="searchBox">
    <uni-nav-bar :title="$t('全部游戏')" leftIcon="back" 
    :status-bar="true" :fixed="true" :shadow="false"
    @clickLeft="navBack"></uni-nav-bar>
    <view class="game_tab">
      <scroll-view class="game_nav" 
          :enable-flex="true"
        ...
        <view
          ...
          :key="index"
          :class="navIndex == item.id ? 'con-active' : ''"
    ...
    <view class="search_tab">
      <view class="searchLeft" :class="[listLength > 0 ? 'searchTsTop' : '']">
        ...
        <image
          @click="textTrim()"
          class="inputClearn"
          src="@/static/image/indexImg/cha4.png"
          mode=""
        ></image>
      ...
      <view class="game_vendor" v-if="navIndex != 9">
        <zb-tooltip placement="bottom" ref="toolTip">
          <view class="vendor_list" :class="{'isSelcet' : vendorId}">
            <view class="titleName">{{ vendorName }}</view>
            <uni-icons type="arrowdown" size="14" color="#70707a"></uni-icons>
          </view >
          <template v-slot:content>
              <view v-if="menuList && menuList.children.length > 0" class="search_vendor">
                <scroll-view
                  scroll-y="true"
                  class="scroll-Y vendorScroll"
                  @scrolltolower="lower">
                  <view class="vendorList"
                  @tap="onCheckVendor('all')"
                  >
                    <uni-icons :type="!vendorId ? 'checkbox-filled' : 'checkbox'" size="20" color="#ccc"></uni-icons>
                    <view class="tsListLeft"> {{ 'game provi' }}</view>
                  </view>
                  <view class="vendorList"
                    v-for="(item, index) in menuList.children"
                    :key="index"
                    @tap="onCheckVendor(item)"
                  > 
                    <uni-icons :type="vendorId == item.id ? 'checkbox-filled' : 'checkbox'" size="20" color="#ccc"></uni-icons>
                    <view class="tsListLeft"> {{ item.name }}</view>
                  </view>
                </scroll-view>
              </view>
          </template>
    ...
    <view v-else class="seacch_content">
            ...
            <image
              :src="
                item.pictureUrl
                  ...
                  : searchNoneImg
              "
              :class="item.pictureUrl ? '' : 'searchBgImg'"
              mode="widthFix"
...
export default {
  ...
  onLoad(option) {
    this.historyData = uni.getStorageSync("historyData")
      ...
      : [];
    if(option.index){
      this.navIndex = Number(option.index)
      this.viewId = 'id' + this.navIndex
      this.allMenuList = cache.get('game_menus').slice(1)
      this.getMenuList()
      this.getData();
    }
  ...
  methods: {
    getMenuList(){
      ...
      menuList.forEach(i => {
        if(i.id == this.navIndex){
          this.menuList = i
        }
      })
      console.log(this.menuList,this.navIndex,'this,.menliust')
      this.gameKindId = this.menuList.id
    },
    ...
    onCheckVendor(item){
      if(item == 'all'){
        this.vendorId = ''
        this.vendorName = 'game provi'
      }else{
        this.vendorName = item.name
        this.vendorId = item.type == 1 ? item.ids : item.type == 2 ? item.url : item.id
      }
      this.dataList = [];
    ...
    getData() {
      ...
      let self = this;
      let req = {
        gameKindId: this.gameKindId == 9 ? 1 : this.gameKindId,
        status: 1,
        currentPage: this.pageNo,
        pageSize: this.pageSize,
        vendorId: this.gameKindId == 9 ? 2000 : this.vendorId,
        gameName: this.val,
      }
      if(!req.gameName || req.gameName == null){
        delete req.gameName
      }
      if(!req.vendorId || req.vendorId == null){
        delete req.vendorId
      }
        ...
        function (err, res) {
          if (err) {
            ...
            if (isIos === "ios") {
              ...
              if (res.status === 1) {
                   if(window.projectImgUrl === 'bgga' && localStorage.getItem('fbPixelId')){
                      fbq('trackCustom', 'enterGame');
                  }
                  if(window.isMaskApp){
                    maskAppToNative('enterGame')
                  }
                setTimeout(function () {
                  //这里使用setTimeout非常重要，没有将无法实现
