<template>
    <div class="addbankpage">
        ...
        <div>
            ...
            <el-row class="elrows" v-if="bankType == 1">
                <el-col :span="24">
                    <div class="col-title">
                        <span>{{$t('链路')}}：</span>
                    </div>
...
export default {
    ...
    watch:{
        ...
        //     if(this.bankCardInfo.name == '数字货币' || this.bankCardInfo.name == 'GOPAY钱包'){
        ...
        //         this.$confirm('您还未绑定手机，是否去绑定手机？', '提示', {
        ...
        //             })
        //         }).catch(() => {
        //             this.$message.warning('未绑定手机无法绑卡，请先绑定手机')
        //         })
        //     }
        // }
    },
    created(){
        this.bankType = this.$route.query.type
        this.getBankList();
        this.getuserInfo();
    ...
    methods: {
            ...
            this.$http.get(this.$api.querybank).then((res) => {
                if (res.code == 0) {
                    this.bankNameList = res.data.filter(item =>{
                        return item.type == this.bankType
                    });
                    // let selectdata = this.bankNameList.filter((item) => {
                    //     return item.name === '数字货币';
                    // });
                    // this.backImg = this.$common.getImgUrl(selectdata[0].imgUrl);
                }
            });
        ...
        getPhoneCode() {
            ...
            this.$http
                .get(this.$api.sendSmsCode + "/" + this.bankCardInfo.phoneCode +`?functionId=7&codePrefix=${this.$config.codePrefix}&codeFlag=1`)
                .then((res) => {
                    if (res.code == 0) {
                        this.$message.success($t("获取验证码成功"));
        ...
        onChangeBankNumber(e) {
            ...
            // pattern.test(bank)
            if (!bank.length) {
                this.bankNumberTipsText = this.$t('请输入正确的银行卡号');
        ...
        submitBankInfo() {
            let option = null;
                ...
                if(!this.bankCardInfo.branch && this.bankType == 1){
                    this.$message.error(this.$t("请选择链路"));
                    return;
                }
                if(!this.bankCardInfo.number){
