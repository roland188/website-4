<template>
    <div class="play">
        <div class="playList">
            ...
            <!-- 顶部背景图片 -->
            <div class="bg">
                <img loading="lazy" class="img" v-lazy="$config.getLocaleImg('listbg6','jpg')" alt="">
            </div>
            <!-- 游戏二级列表 -->
            ...
            <div class="showPlayList">
                ...
                <ul style="overflow:hidden;margin-top:15px;">
                    <li class="ele-card-box has-link" v-for="(item1, ind) in gameList" :key='ind' @click="getToken(item1, 1)">
                        <div class="img-wrap" :class="{ 'img-wrap1': item1.status == 0 }">
                            <img loading="lazy"
                                v-lazy="item1.pictureUrl ? ($config.imgHost + item1.pictureUrl) : item1.imgUrl ? ($config.imgHost + item1.pictureUrl) : ''"
                                :onError="noData" />
                            <div class="mask">
                ...
                <div 
                    style="display:flex;width:1200px;height:400px;color:white;justify-content:center;align-items:center;letter-spacing:2px;font:18px/36px normal;"
            ...
            <div>
                <el-pagination background :page-size='dataInfo.pageSize' :current-page="dataInfo.curPage"
                    @current-change="changePage" layout="prev, pager, next" :total="dataInfo.total">
                </el-pagination>
            </div>
        </div>
    </div>
...
export default {
    ...
    mounted() {
        ...
        // 赋值 总列表
        this.menuList = JSON.parse(localStorage.getItem("ALLMENUE_EXCEPT_FISH")).filter(v => {return v.id == 7});
    ...
    methods: {
        ...
        getToken: async function (req, index) {
            ...
            if (req.status === 0) {
                return self.$message.error(self.$t('维护中'))
            }
