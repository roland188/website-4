  ...
  <div class="themeColor index">
    ...
    <div class="main">
      <div
        class="leftMenu"
        ref="leftMenuBar"
        @mousemove="leftMenuHover"
        @mouseleave="leftMenuLeave"
      >
        <menu-list
          @setregister="$emit('openRegister')"
          @scrollToCode="$emit('scrollToCode')"
        ></menu-list>
      </div>
    ...
    >
      <p>
        {{
          $t("系统检测到您的账户登录地区异常，请填写您的账户真实姓名进行验证！")
        }}
      </p>
      <div class="name">
        <span>{{ $t("姓名") }}：</span>
      ...
      <div class="operation">
        <el-button class="submitValidation" type="primary" @click="submitName()"
          >{{ $t("提交验证") }}
        </el-button>
        <el-button class="returnLogin" @click="dialogFormVisible = false">{{
          $t("返回登录")
        }}</el-button>
      </div>
    </el-dialog>
...
export default {
  ...
  mounted() {
    ...
    if (
      ...
      window.location?.search.includes("isShowRegister")
    ) {
      this.openRegister();
    }
  },
  methods: {
    ...
    leftMenuHover() {
      let homeView = this.$refs.homeView;
      homeView.style.width = "calc(100% - 2.3rem)";
      homeView.style.marginLeft = "2.3rem";
    },
    leftMenuLeave() {
      let homeView = this.$refs.homeView;
      homeView.style.width = "calc(100% - 0.66rem)";
      homeView.style.marginLeft = "0.66rem";
    },
    //校验真实姓名
    submitName() {
    ...
    .best_inner {
      ...
      .title {
        ...
        .service_online {
          ...
          width: 180px;
          height: 50px;
          background: url(../../assets/image/gameImg/index/btn_service.png) 50%
            0 no-repeat;
          &:hover {
            background-position: 50% 100%;
          }
