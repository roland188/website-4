<template>
    <div class="header_container">
        ...
        <div class="headerWidth">
            <div class="headerLeft">
                ...
                <!-- <el-select v-model="selLang" :placeholder="$t('请选择')" size="small" class="lang-s" popper-class="lang-p" @change="handleSelectLang">
                    ...
                    <el-option
                        v-for="(item, i) in langOpts"
                        :key="i"
                        :value="item.languageCode"
                    >
                        <img loading="lazy" v-lazy="$common.getImgUrl(item.countryFlag)" width="20px" style="margin-right: 12px" />
                ...
                <ul>
                    ...
                    <!-- <template v-if="codeshow">
                        <li v-for="(item, index) in headerList" :key="index" @click="jump(item)" class="liCenter">
                                <i
                                    v-show="item.hotStatus"
                                    :style="
                                        'background-image:url(' +
                                        $common.getImgUrl(item.hotPic) +
                                        ')'
                                    "
                                ></i>
                                <span
            ...
            <div class="headerRight" v-show="!isLogin">
                <!-- <input type="text" :placeholder="$t('账号')" v-model="account" @blur="accountBlur" />
                <div class="password">
                    <input type="password" :placeholder="$t('密码')" v-model="pwd" @blur="passwordBlur" />
                    <span @click="to('pwdforget')">{{ $t('忘记') }}？</span>
                </div>
                <div v-show="showPicture" class="defaultPicture">{{ $t('获取中') }}</div> -->
                <div
                    v-show="!showPicture"
                ...
                >
                    <!-- <template v-if="type == 0">
                        ...
                        <div
                            ...
                            @click="captchaData"
                        >
                            <img loading="lazy" v-lazy="require('../../../../assets/image/refresh.png')" class="refimg" alt />{{ $t('重新获取') }}
                        </div>
                    </template> -->
                    ...
                    <!--  行为验证 极验 -->
                    <!-- <div class="logRow" v-if="type === 1">
                        <div v-show="type === 1" id="captcha1" class></div>
                        <div v-if="errorCode" class="getCode" @click="getDefaultStatus">
                            <img loading="lazy" v-lazy="require('../../../../assets/image/refresh.png')" class="refimg" alt />{{ $t('重新获取') }}
                        </div>
                    </div> -->
                    <!-- 极验4 -->
                    <!-- <div v-if="type === 3" id="gt4"></div> -->
                </div>
                ...
                <div class="rightButton">
                    ...
                    <!-- <div class="rightButtonOne" @click="$emit('openRegister')"> -->
        ...
        <!-- eslint-disable-next-line vue/custom-event-name-casing -->
        <menu-list class="menulist" @setregister="$emit('openRegister')" @scrollToCode="$emit('scrollToCode')"></menu-list>
...
import { changeLang, langObj } from '@/i18n'
...
export default {
    ...
    data() {
        return {
            ...
            langOpts: [
                { languageName: 'Trang chủ', languageAbbr: 'TC', languageCode: 'vi', icon: 'tc' },
                { languageName: 'English', languageAbbr: 'EN', languageCode: 'en', icon: 'en' },
                { languageName: '中文-简体', languageAbbr: '简', languageCode: 'zh_CN', icon: 'zh_cn' },
                // { languageName: '中文-繁体', languageAbbr: '繁', value: 'zh_TW', icon: 'zh_hk' },
            ],
    ...
    mounted() {
        ...
        //     var dd =
        //         date.getDate() < 10 ? "0" + date.getDate() : date.getDate(); //天
        //     var hours =
    ...
    methods: {
        ...
        async getLangOpts() {
            const res = await this.$http.get(this.$api.getLangList)
            if (res.code === 0) {
                this.langOpts = res.data
                this.getLang()
            }
        },
        // 获取语言
        getLang(l) {
            ...
            const { languageName } = this.langOpts.find(o => l ? l === o.languageCode : langObj[o.languageCode] === local) || {}
            if (languageName) {
                this.selLang = languageName
            }
        },
        // 换语言
        handleSelectLang(v) {
            this.getLang(v)
            const { languageCode } = this.langOpts.find(o => o.languageCode === v) || {}
            languageCode && changeLang(languageCode)
        },
        // 客服问题
        async getServer(){
            ...
            console.log(res.maintainUrl,"ddddd",res)
        },
        getTime () {
        ...
        jump(item) {
            if (item.editStatus === 0) {
                ...
                // } else {
                //     this.$message.error(this.$t('vip.gotologin'));
                //     this.$store.commit("loginNavId", 0);
                //     showLogin({
                //         show: true
                //     });
...
.lang-p {
    ...
    .el-select-dropdown__item.hover, .el-select-dropdown__item:hover {
        background-color: 	rgba(225,179,105, .2);
        color: #fff;
    }
    .selected {
        color: #fff !important;
