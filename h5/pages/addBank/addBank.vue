...
<template>
  <view class="addBankPage">
    <uni-nav-bar :title="$t('添加银行卡')" leftIcon="back" :status-bar="true" :fixed="true" :shadow="false" @clickLeft="BackPage"></uni-nav-bar>
    ...
    <view class="conent pageBackground">
      <view class="addCardUl">
        ...
        <view class="uni-flex border-item" v-if="(bankCodeName == $t('数字货币') && ['sovip','betc88'].includes($config.projectImgUrl))
          || !['sovip','betc88'].includes($config.projectImgUrl)">
          <view class="flex-item">
    ...
    <uni-popup ref="popup" type="bottom" @tap.stop @change="handlePopupChange">
      ...
      <view class="padding-xl">
        <view class="bank-search">
					<input class="bank-search__input" type="text" :cursor-spacing="400" v-model.trim="keyword" :placeholder="$t('请输入银行名称')" placeholder-style="color:#999;font-size:28rpx">
...
export default {
  ...
  onLoad() {
    this.$api.getbanklist((err, res) => {
      // this.items = res;
      this.items = res.filter(item => { return item.name !== this.$t('数字货币') })
    });
  },
  onShow() {
    const self = this
  ...
  methods: {
    ...
    submit() {
      ...
      if(!['sovip','betc88'].includes(this.$config.projectImgUrl)){
      ...
      var data = {
        ...
        branch: ['sovip','betc88'].includes(this.$config.projectImgUrl) ? this.bankData.name : this.bankData.branch, //	支行
