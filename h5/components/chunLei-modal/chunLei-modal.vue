	 ...
	 @touchmove.stop.prevent>
	  ...
	  @touchmove.stop.prevent> -->
		...
		<block v-if="type=='update'">
			<view class="share-view" @tap.stop>
				<view class="share-box">
					<image src="../../static/image/updataapp.png" mode="widthFix" class="indexTanTop"></image>
					<view class="content">
				...
				<image class="image cancel" v-if="mData.cancel == 0" @tap.stop="tapCancel" src="../../static/image/cha4.png"></image>
			</view>
		...
		<block v-if="type=='share'">
			<view class="share-view" @tap.stop>
				<view class="share-box clBox">
					...
					<swiper class="swiper" @change="change" autoplay="true" interval='6000' :indicator-dots="true">
				...
				<image class="image cancel" @tap.stop="tapCancel" src="../../static/image/tNone.png"></image>
			</view>
		</block>
		...
		<block v-if="type=='userVip'">
			<view class="uservip" @tap.stop>
				<view class="uservip-box">
					<image src="../../static/image/bgviptitle.png" mode="widthFix" class="indexTanTop"></image>
					<view class="">
				...
				<image class="image cancel" @tap.stop="tapCancel" src="../../static/image/cha4.png"></image>
		...
		<block v-if="type=='userback'">
			<view class="user-back" @tap.stop>
				...
				<view class="btn newBtn">
					<view class="button cancel themeTextTwo" v-if="mData.cancelText" @tap="tapCancel(mData.cancelText)">{{mData.cancelText?mData.cancelText:$t('取消')}}</view>
		...
		<block v-if="type == 'advertising'">
			<slot>
				<view class="home-placard">
					<!-- 弹窗内容区 -->
					<view class="home-placard-main">
						<swiper
							class="swiper"
							:indicator-dots="true"
						...
						<view @tap.stop.prevent="tapCancel" class="home-alert-close"></view>
					</view>
					<!-- 关闭按钮 -->
				</view>
			</slot>
			...
			jump(item) {
				...
				if (item.type === 1) {
					...
					if (this.$api.isLogin() && item.bannerGame.id) {
						...
						this.$emit('enterGame', item.bannerGame)
					} else {
						uni.showToast({
							title: "请先登录",
							icon: "none",
			...
			pullKing(rules) {
				...
				if (rules.popUpSound === 1) {
					setTimeout(() => {
						...
						innerAudioContext.src = '../../static/image/coin.mp3';
...
<style lang="scss">
	.pullKing {
		...
		.content {
			...
			.pullKingClose {
				...
				height: 90rpx;
				border-radius: 50%;
				background: url('../../static/image/cha4.png') no-repeat;
				background-size: 100% 100%;
				color: #ffffff;
				display: flex;
				justify-content: center;
	...
	.clBox {
		background: url('../../static/image/t3-1.png') no-repeat;
	...
	.home-placard {
		...
		.home-placard-main {
			...
			.home-alert-close {
				width: 80upx;
				height: 80upx;
				background: url("../../static/image/tNone.png") no-repeat center top;
				background-size: 100%;
				position: absolute;
				bottom: -100upx;
				left: 50%;
			...
			.content {
				...
				padding: 0upx;
				// background: url('@/static/img/req/h5_bg.png') repeat-y center center;
				// background-size: 100% 100%;
				border-radius: 24upx;
				margin-bottom: 40upx;
				color: #fcf4e0;
