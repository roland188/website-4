<template>
  <view class="s-pull-scroll" :class="customClass">
    ...
    >
      <view :style="{ transform: translateY, transition: transition }">
        ...
        <slot v-else-if="isFinish" name="finish">
          <view class="s-pull-tip-wrap" v-if="finishText && isShowFinishText">{{
            finishText
          }}</view>
        </slot>
      </view>
    ...
    <view
      v-if="backTop"
      :class="['s-pull-back-top', { show: isShowBackTop }]"
      @click="onBackTop"
    >
      <slot name="backtop">
        <view class="default-back-top"><img src="./back-top.png" /></view>
      </slot>
    </view>
  </view>
</template>
...
export default {
  ...
  computed: {
    ...
    fixedTop() {
      if (this.head == 3 || this.head == 3.1 || this.head == 3.2) {
        // #ifdef APP-PLUS
        return this.fixed ? "132px" : 0;
        // #endif
        // #ifdef H5
        return this.fixed ? "88px" : 0;
        // #endif
      } else {
        if (this.tops) {
          // #ifdef APP-PLUS
          return this.fixed ? "144px" : 0;
          // #endif
          // #ifdef H5
          return this.fixed ? "100px" : 0;
          // #endif
        } else {
          // #ifdef APP-PLUS
          return this.fixed ? "88px" : 0;
