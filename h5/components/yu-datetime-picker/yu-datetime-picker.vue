	...
	<view class="yu-datetime-picker">
		...
		<view class="yu-datetime-content" :class="{'show':showPicker}">
			<view class="yu-datetime-hd" @touchmove.stop.prevent catchtouchmove="true">
				...
				<view class="timeTitle recordTextOne">{{ $t('选择时间') }}</view>
	...
	export default {
		...
		data() {
			...
			return {
				...
				second,
				dateObj,
				itemHeight: `height: ${uni.upx2px(100)}px;`,
				values: [0, 0, 0, 0, 0, 0],
				selectArr: [],
				selectRes: "",
				showPicker: false,
		...
		methods: {
				...
				if (_this.value) {
					...
					if (_this.isAll) {
						...
						if (str < localStr) {
							...
							_this.selectRes =`${_this.formatNum(_this.year)+'-' + _this.formatNum(_this.month)}`;
						} else {
			...
			showCurrent() { //显示当前的日期时间
				...
				this.selectRes = `${this.formatNum(this.year)+'-'+this.formatNum(this.month)+'-'+this.formatNum(this.day)}`;
				if (this.isAll) {
...
<style>
@import url("./yu-datetime-picker.css");
</style>
