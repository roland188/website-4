<template>
    <div class="securityCenter personal">
        <!-- 交易密码 -->
        <!-- 初次设置 -->
        <div>
            ...
            <!-- 提交 -->
            <div class="perSubmit"  v-if="!withdrawPassWordState">
              <div class="u-flex-all themeColorkBgc" @click="setTradePassword">
                <el-button type="primary" round style="width: 100%;"  :disabled='setTradePwdDisabled ? false :true'>{{$t('提交')}}</el-button>
              </div>
            </div>
        </div>
        <!-- 修改交易密码 -->
        <div>
            ...
            <!-- 提交 -->
            <div class="perSubmit" v-if="withdrawPassWordState">
              <div  class="u-flex-all themeColorkBgc" @click="updateTradePassword">
                <el-button type="primary" round style="width: 100%;" :disabled='upTradePwdDisabled  ? false :true'>{{$t('提交')}}</el-button>
              </div>
            </div>
        </div>
    </div>
...
export default {
  ...
  methods: {
      getMemberInfo: async function() {
        ...
        if (res.code == 0) {
            this.withdrawPassWordState = res.data.withdrawPassWordState;
        }
    },
    tradePasswordIsValid() {
      //交易密码校验
      return /^[0-9]{6}$/.test(this.tradePassword);
    },
    traPwdIsConfirmPwd() {
      //交易密码和确认密码是否相同
      return this.tradePassword == this.confirmTradePwd;
    },
    newTradePasswordIsValid() {
      //交易密码校验
      return /^[0-9]{6}$/.test(this.newTradePassword);
    },
    newTraPwdIsConfirmPwd() {
      //新的交易密码和确认密码是否相同
      return this.newTradePassword == this.confirmNewTradePwd;
    },
      //设置交易密码
    async setTradePassword() {
      var _this = this;
      if (!this.setTradePwdDisabled) {
        return;
      }
      //校验
      if (!this.tradePasswordIsValid()) {
        this.$message.error(this.$t("交易密码必须为6位数字"));
        return;
      } else if (!this.traPwdIsConfirmPwd()) {
        this.$message.error(this.$t("交易密码与确认密码不一致！"));
        return;
      }

      //设置交易密码接口
      await _this.setTradePasswordInter();
    },
        //修改交易密码
    async updateTradePassword() {
      var _this = this;
      if (!this.upTradePwdDisabled) {
        return;
      }
      //校验
      if (!this.newTradePasswordIsValid()) {
        this.$message.error(this.$t("交易密码必须为6位数字"));
        return;
      } else if (!this.newTraPwdIsConfirmPwd()) {
        this.$message.error(this.$t("新的交易密码与确认密码不一致！"));
        return;
      }
      //修改交易密码接口
      await _this.updateTradePasswordInter();
    },
      //设置交易密码接口
    async setTradePasswordInter() {
      var _this = this;
      var data = {
        withdrawpassword: this.tradePassword
      };
      var res = await _this.$http.post(_this.$api.withdrawPassSet, data);
      if (res.code == 0) {
        this.$message.success(this.$t("设置交易密码成功"));
        this.tradePassword = ''
        this.confirmTradePwd = ''
      } else {
        this.$message.error(res.msg);
      }
    },
      //修改交易密码接口
    async updateTradePasswordInter() {
      var _this = this;
      var data = {
        newWithdrawpassword: this.newTradePassword,
        withdrawpassword: this.oldTradePassword
      };
      var res = await _this.$http.post(_this.$api.withdrawPass, data);
      if (res.code == 0) {
        this.$message.success(this.$t("修改交易密码成功"));
        this.getMemberInfo()
        this.oldTradePassword  = ''
        this.newTradePassword =''
        this.confirmNewTradePwd =''
      } else {
        this.$message.error(res.msg);
      }
    },
...
<style >
   ...
   .securityCenter .el-collapse-item__header{
       padding: 0 0.16rem;
   }
</style>
<style scoped>
