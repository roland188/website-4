...
<template>
    <view class="content">
        ...
        <view class="container" :class="{'p-bottom-40': isAndroid}">
            <!-- 功能登录区域 -->
            <LoginBox ref="loginBox" @routerLink="routerLink" :login="login" class="pad-common"/>
            <!-- 公告 -->
            <NoticeBar @openalert2="openalert2" @updateNotice="e => modeldata.notices = e" class="pad-common"/>
            <!-- 熱門 常玩 推薦 tab -->
            <!-- <HomeTabs ref="HomeTabs" :hotList="hotList" :tuiList="tuiList" :changList="changList" :goToGame="difference" :isLogin="login" /> -->
            <!-- 游戏类型 -->
            <GameType></GameType>
            <!-- 游戏菜单 -->
            <GameList class="pad-common" ref="gameList" :arr="arr" @difference="difference" @changeRightData="changeRightData" :leftArray="leftArray" :paysList="paysList" :gamemenusparent="gamemenusparent" :leftMenuIcon="leftMenuIcon" />
            <view class="rich-bg"></view>
            <PrizePool class="pad-common"></PrizePool>
            <OtherInfo class="pad-common"></OtherInfo>
        </view>
...
export default {
  ...
  methods: {
    getAllKindGames(){
      ...
      // 6  Esports  电子竞技
      // 7  Fishing  捕鱼
      this.$api.allKindGames(params, (err, res) => {
        this.paysList = Object.assign([],res)
      })
    },
    //
    updateLoadData() {
    ...
    difference(item,type) {
      ...
      // 此状态跳转到列表
        if (index == 0) {
          this.dataClick1(item, 1);
        } else {
          this.dataClick1(item, 2);
        }
      // #endif
    },
    // 跳转详情 bmw
    ...
    routerWeb(item, type) {
      ...
      // 此状态跳转到列表
      if (type == 1 || type == 3) {
        this.dataClick(item, 2);
      }else{
        this.dataClick(item, 1);
      }
    },
    // 常玩游戏
    ...
    getadd() {
      ...
      this.$cache.set('game_menus',arrLeft)
    ...
    routerLink(type, id, subType) {
      ...
      if (type == 1) {
        ...
        if (subType == 2) {
          ...
          title = "签到";
          if (this.$api.isLogin()) {
            obj.memberId = this.$cache.get("set_user").user_id;
          } else {
            uni.navigateTo({
              url: "/pages/login/login",
            });
...
.content {
  ...
  .container {
    ...
    /* #endif */
    .pad-common{
      padding: 4upx 20upx 0;
    }
    .rich-bg{
      width: 100%;
      height: 554upx;
      background: url("~@/static/image/indexImg/rich-bg.gif") no-repeat;
